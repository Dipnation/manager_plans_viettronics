$(document).ready(function(){
	...
	/** set date, time cho form them lich cong tac **/
	$('#date1').datepicker();
	$('#time1').timepicker()
...
$(document).ready(function() {
	...
	$('#calendar').bootstrapValidator({
		...
		fields: {
			cal_content: {
				message: 'Nội dung công việc không hợp lệ',
				validators: {
					notEmpty: {
					...
					stringLength: {
						min: 6,
						max: 200,
						message: 'Nội dung công việc bị giới hạ 6-200 ký tự'
					}
				}
			},
			account_manager: {
				validators: {
					notEmpty: {
						message: 'Tên người chủ trì không được để trống'
					}
				}
			},
			unit_id: {
			...
			cal_join: {
				validators: {
					notEmpty: {
						message: 'Bạn chưa nhập thành phần tham dự'
					},
