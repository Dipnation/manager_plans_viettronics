					...
					if ($_FILES['avatar']['error'] == 0) {	           
						if ($_FILES['avatar']['size'] > $max_file_size) {
	    					...
	    					$errors[] = 'File không đúng định dạng!.';
								...
								//kiem tra neu nguoi dung cap nhat mat khẩu
								if(!empty($_POST['pass'])){
									...
									if(count($errors) == 0){
										...
										$mysqli->query("UPDATE `account` SET `email` = '$email',`pass`='$pass' `fullname` = '$fullname', `unit_id`='$unit',`status`='$status',`avatar`='$name' WHERE `id` = '$id'");
									...
									move_uploaded_file($_FILES["avatar"]["tmp_name"], $path.$name);
									//cap nhat bang account
									$mysqli->query("UPDATE `account` SET `email` = '$email', `fullname` = '$fullname', `unit_id`='$unit',`status`='$status',`avatar`='$name' WHERE `id` = '$id' ");
									$msg[] = 'Cập nhật thành công!';
								}	
