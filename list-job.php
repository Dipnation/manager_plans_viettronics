<?php
	...
	$stt =1;
	//xu ly del don vi
	if($_SESSION['status'] == 3 && isset($_POST['submit_del_job'])){
		$id = $_POST['id'];
		$sql_del = $mysqli->query("DELETE FROM `job` WHERE `id` = '$id'");
	}
...
<!-- BEGIN PAGE CONTENT -->
	<div class="page-content">
		...
		<ol class="breadcrumb default square rsaquo sm">
			...
			<li><a href="#fakelink">Nhóm công việc</a></li>
		...
		<?php if($_SESSION['status']==3 && !isset($_GET['id'])){
			...
			$sql = $mysqli->query("SELECT * FROM `job_info`");
		?>
		<h4 class="small-title">Danh sách nhóm công việc</h4>
		<div class="the-box">
			<div class="table-responsive">
				<table id="user3" class="table table-th-block table-primary">
					<thead>
						<tr>
							<th width=6%>No</th>
							<th><center>Nhóm CV</center></th>
							<th><center>Đơn vị</center></th>
							<th width=39%><center>Mô tả</center></th>
					...
					<?php 
						...
						echo '<tr>
							...
							<td><center><div class="btn-group">
								  	<span data-toggle="tooltip" title="" data-original-title="Chỉnh sửa"><a href="'.$base_url.'edit-job-'.$obj->id.'.html"><i class="fa fa-pencil icon-square icon-xs icon-primary"></i></a></span>
							...
							<div id="text-popup-'.$obj->id.'" class="danger-popup mfp-with-anim mfp-hide">
								<div class="row">
								<center><p>Bạn có chắc chắn muốn xóa nhóm công việc:<br>';
								echo'</p></center>
								<form method="POST">
									...
									<div class="col-xs-3">
										...
										<button type="submit" name="submit_del_job" class="btn btn-primary">Đồng ý</button>
